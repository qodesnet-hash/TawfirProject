...
class APITester:
    ...
    def test_merchant_status_validation(self):
        """اختبار: التحقق من حالات التاجر المختلفة"""
        self.print_header("🔐 اختبار: حالات التاجر المختلفة")
        
        statuses = ['قيد المراجعة', 'مقبول', 'مرفوض']
        original_status = self.test_merchant.status
        
        for test_status in statuses:
            try:
                self.test_merchant.status = test_status
                self.test_merchant.save()
                ...
                offer_data = {
                    'title': f'عرض اختبار - {test_status}',
                    'description': 'وصف',
                    'price_before': '100',
                    'price_after': '50',
                    'city': str(city.id),
                ...
                request = self.factory.post('/api/v1/merchant/offers/create/', data=offer_data)
                force_authenticate(request, user=self.test_user)
                
                view = MerchantOfferCreateView.as_view()
                response = view(request)
                
                if test_status == 'مقبول':
                    expected = 201
                ...
                self.print_result(f"حالة '{test_status}'", False, str(e))
        
        # إعادة الحالة الأصلية
        self.test_merchant.status = original_status
        self.test_merchant.save()
    
    def print_summary(self):
        """عرض ملخص النتائج"""
        self.print_header("📋 ملخص النتائج")
        ...
        if percentage == 100:
            ...
            print("\n⚠️  العديد من الاختبارات فشلت - يحتاج مراجعة")
    ...
    def run_all_tests(self):
        ...
        self.test_merchant_status_validation()
